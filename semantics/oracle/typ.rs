...
impl<'a> Oracle<'a> {
    ...
    fn lower_response(&mut self, resp: &ast::TypeResponse) -> Option<TypeId> {
        ...
        // Is response body valid. We don't want to early return, because
        // we want to validate as many things as possible and capture as many
        // errors as possible.
        //
        // In previous checks for body we do want early return, since there is nothing more we can
        ...
        if requires_file {
            ...
            match self.require_no_file(&body, resp.content_type().as_ref()) {
                Ok(_) => (),
                Err(_) => is_valid = false,
            }
        }
    ...
    fn require_no_file(
        ...
        match node {
            ...
            ast::Type::TypeOption(opt) => match opt.typ() {
                Some(typ) => self.require_no_file(&typ, content_type),
                None => Err(()),
            },
            ...
            ast::Type::Record(record) => {
                let valid = record.fields().all(|field| {
                    field
                        .typ()
                        .is_none_or(|typ| self.require_no_file(&typ, content_type).is_ok())
                });
            ...
            // Response should not be part of response type, which is what this function is
            // validating. Errors for this are reported later, but we should return Err(())
            // here.
            ...
            ast::Type::TypeRef(reference) => {
                ...
                match self.resolve(name_id, reference.syntax().text_range()) {
                    ...
                    ResolvedSymbol::Type(typ) => self.require_no_file(&typ, content_type),
                }
            }
