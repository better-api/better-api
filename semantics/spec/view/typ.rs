...
impl Spec {
    ...
    ) -> NamedRootTypeRefView<'a> {
        let RootTypeData::Reference(data) = self.types.get_root_type(id.id()) else {
            unreachable!("ResponseReferenceProof must point to a RootReferenceData type slot");
        };
